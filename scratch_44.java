...
class Scratch {
    ...
    public String largestNumber(int[] cost, int target) {
        ...
        for (int i = 0; i < 10; i++) {
            dp[i][0] = true;
            m.put(new Pair<>(i, 0), new LinkedList<>());
            m.get(new Pair<>(i, 0)).add(new LinkedList<>());
        }
        for (int i = 1; i <= 9; i++) {
        ...
        boolean targetCom = false;
        for (int i = 0; i < 10; i++) {
            targetCom |= dp[i][target];
        }
        if (!targetCom) return "0";
            ...
            for (int j = 1; j <= target; j++) {
                if (dp[i][j]) {
                    ...
                    if (j - cost[i - 1] == 0) {
                        ...
                        for (int k = 1; k <= i; k++) {
                            if (dp[k][j - cost[i - 1]]) {
                                Pair<Integer, Integer> prev = new Pair<>(k, j - cost[i - 1]);
                                for (List<Integer> li : m.get(prev)) {
                                    tmp.add(new LinkedList<>(li));
                    ...
                    if (tmp.size() == 0) {
                        ...
                        tmp.sort(Comparator.comparingInt(o -> -o.size()));
                        int maxSize = tmp.get(0).size();
                        ...
                        for (List<Integer> il : tmp) {
                            il.sort(Comparator.comparingInt(o -> -o));
                        }
                        tmp.sort((o1, o2) -> {
                            Iterator<Integer> it1 = o1.listIterator();
                            Iterator<Integer> it2 = o2.listIterator();
                            while (it1.hasNext()) {
                                int n2 = it2.next();
                                int n1 = it1.next();
                                if (n1 - n2 > 0) return -1;
        ...
        for (int i = 0; i < 10; i++) {
            if (dp[i][target]) {
                List<List<Integer>> tmp = m.get(new Pair<>(i, target));
                for (List<Integer> l : tmp) {
                    if (l.size() > resultList.size()) {
                        resultList = l;
                    } else if (l.size() == resultList.size()) {
                        Iterator<Integer> lIt = l.listIterator();
                        Iterator<Integer> rIt = resultList.listIterator();
                        while (lIt.hasNext()) {
                            int lNum = lIt.next();
                            int rNum = rIt.next();
                            if (lNum > rNum) {
