...
class scratch_55 {
    ...
    public int minSwaps(String s) {
        ...
        if (freq[0] == freq[1]) {
            int prefixZeroDiff = 0, prefixOneDiff = 0;
            for (int i = 0; i < ca.length; i++) {
                // prefixZero
                if (ca[i] - '0' != (i & 1)) prefixZeroDiff++;
                else prefixOneDiff++;
            }
            return Math.min(prefixOneDiff, prefixZeroDiff) / 2;
    ...
    // LC1669
    public ListNode55 mergeInBetween(ListNode55 list1, int a, int b, ListNode55 list2) {
        ListNode55 dummy = new ListNode55(-1);
        dummy.next = list1;
        if (a == 0) dummy.next = list2;
        ListNode55 prev = null, cur = list1;
        ListNode55 firstHalfEnd = new ListNode55(-1), secondHalfStart = new ListNode55(-1);
        int idx = 0;
        for (int i = 0; i <= b; i++) {
            if (idx == a) {
                firstHalfEnd = prev;
    ...
    // JZOF II 022 LC142
    public ListNode55 detectCycle(ListNode55 head) {
        // a b c
        // b+c = 环长
        // slow fast 在 b 相遇
        ...
        ListNode55 slow = head, fast = head;
        while (fast != null) {
            fast = fast.next;
    ...
    public String smallestFromLeaf(TreeNode55 root) {
    ...
    private void lc1883Helper(TreeNode55 root) {
    ...
    // LC445
    public ListNode55 addTwoNumbers(ListNode55 l1, ListNode55 l2) {
        ListNode55 h1 = reverseList(l1), h2 = reverseList(l2);
        ListNode55 p1 = h1, p2 = h2;
        int carry = 0;
        ListNode55 dummy = new ListNode55(-1);
        ListNode55 cur = dummy;
        while (p1 != null && p2 != null) {
            int next = carry + p1.val + p2.val;
            carry = next / 10;
            ListNode55 nextNode = new ListNode55(next % 10);
            cur.next = nextNode;
            cur = cur.next;
            p1 = p1.next;
            p2 = p2.next;
        ...
        if (carry != 0) {
            cur.next = new ListNode55(carry);
            cur = cur.next;
        }
        return reverseList(dummy.next);
    }

    private ListNode55 reverseList(ListNode55 head) {
        if (head == null) return null;
        ListNode55 cur = head, prev = null;
        while (cur != null) {
            ListNode55 origNext = cur.next;
            cur.next = prev;
            prev = cur;
            cur = origNext;
        }
    ...
    public int goodNodes(TreeNode55 root) {
        lc1448Helper(root, Integer.MIN_VALUE);
        return lc1448Result;
    }

    private void lc1448Helper(TreeNode55 root, int curMax) {
        if (root == null) return;
    ...
    // JZOF II 055
    class BSTIterator {
        Deque<TreeNode55> stack = new LinkedList<>();
        TreeNode55 cur;

        public BSTIterator(TreeNode55 root) {
            cur = root;
        }
    ...
    // JZOF 26
    public boolean isSubStructure(TreeNode55 a, TreeNode55 b) {
        // 空树不是任何树的子结构
        if (a == null || b == null) return false;
        return lc40Helper(a, b) || isSubStructure(a.left, b) || isSubStructure(a.right, b);
    }

    private boolean lc40Helper(TreeNode55 a, TreeNode55 b) {
...
class ListNode55 {
    ...
    ListNode55(int val, ListNode55 next) {
        this.val = val;
        this.next = next;
    }
}
