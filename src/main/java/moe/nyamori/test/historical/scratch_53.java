...
//        System.out.println(s.maximizeXor(new int[]{5, 2, 4, 6, 6, 3}, new int[][]{{12, 4}, {8, 1}, {6, 3}}));
    ...
    // LC450 ** 很漂亮的递归
    public TreeNode53 deleteNode(TreeNode53 root, int key) {
        if (root == null) return null;
        if (root.val == key) {
            if (root.left == null) return root.right;
            if (root.right == null) return root.left;
            // 如果既有左子树 又有右子树, 则用右子树中最小的节点代替root
            TreeNode53 minNode = findMinNode(root.right);
    ...
    private TreeNode53 findMinNode(TreeNode53 root) {
    ...
    // LC776 ** 不太懂
    public TreeNode53[] splitBST(TreeNode53 root, int target) {
        if (root == null) return new TreeNode53[2];
        TreeNode53[] result;
        if (root.val <= target) {
    ...
    // JZOF II 050 LC437  **
    public int pathSumIII(TreeNode53 root, int targetSum) {
        if (root == null) return 0;
        return lc437Helper(root, targetSum) + pathSumIII(root.left, targetSum) + pathSumIII(root.right, targetSum);
    }

    private int lc437Helper(TreeNode53 root, int targetSum) {
        if (root == null) return 0;
        targetSum -= root.val;
    ...
    public int numSimilarGroups(String[] strs) {
        DisjointSetUnion53<Integer> dsu = new DisjointSetUnion53<>();
    ...
    // JZOF II 026 LC143
    public void reorderList(ListNode37 head) {
        ...
        ListNode37 nextHalf = slow.next;
        ListNode37 nextHalfDummy = new ListNode37(-2);
        nextHalfDummy.next = nextHalf;
        slow.next = null;

        // 翻转nextHalf
        ListNode37 prev = null, cur = nextHalf;
        while (cur != null) {
        ...
        ListNode37 reversedNextHalfHead = prev;
        nextHalfDummy.next = reversedNextHalfHead;

        cur = dummy.next;
        while (reversedNextHalfHead != null) {
            ListNode37 origNext = cur.next;
            ListNode37 origRNHH = reversedNextHalfHead;
    ...
    // LC536
    public TreeNode53 str2tree(String s) {
        if (s.equals("")) return null;
        StringBuilder sb = new StringBuilder();
        char[] ca = s.toCharArray();
        boolean number = true, left = false, right = false;
        TreeNode53 root = new TreeNode53();
        int val = -1, pair = 0;
        ...
        for (int i = 0; i < ca.length; i++) {
            ...
            if (number) {
                ...
                if (c == '(') pair++;
                    ...
                    if (pair == 0) {
                        endOfLeft = i;
                        TreeNode53 leftNode = str2tree(s.substring(startOfLeft + 1, endOfLeft));
                        root.left = leftNode;
                        startOfRight = i + 1;
                        right = true;
                        left = false;
                ...
                if (c == '(') pair++;
                    ...
                    if (pair == 0) {
                        endOfRight = i;
                        TreeNode53 rightNode = str2tree(s.substring(startOfRight + 1, endOfRight));
...
class DisjointSetUnion53<T> {

    Map<T, T> father;
    Map<T, Integer> rank;

    public DisjointSetUnion53() {
        father = new HashMap<>();
        rank = new HashMap<>();
    }
...
class BIT53 {
    ...
    public BIT53(int len) {
        this.len = len;
        this.tree = new long[len + 1];
    }

    public BIT53(int[] arr) {
        this.len = arr.length;
        this.tree = new long[len + 1];
