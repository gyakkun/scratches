...
class scratch_52 {
    public static void main(String[] args) {
        scratch_52 s = new scratch_52();
        long timing = System.currentTimeMillis();

//        RangeBit52 rbit = new RangeBit52(10);
...
//        System.out.println(s.removeInterval(new int[][]{{0, 2}, {3, 4}, {5, 7}}, new int[]{1, 6}));
    ...
    // LC337 树形DP
    public int rob(TreeNode52 root) {
        int[] result = lc337Helper(root);
        return Math.max(result[0], result[1]);
    }

    private int[] lc337Helper(TreeNode52 root) {
        // [不偷, 偷]
    ...
    public int longestZigZag(TreeNode52 root) {
        lc1372Helper(root);
        return lc1372Max - 1;
    }

    private int[] lc1372Helper(TreeNode52 root) {
        int[] result = new int[]{0, 0};
        if (root == null) return result;
        // 0-left 1-right
    ...
    public String[] findRestaurant(String[] list1, String[] list2) {
        ...
        for (int i = 0; i < list2.length; i++) {
            if (idxMap1.containsKey(list2[i])) {
                int sum = i + idxMap1.get(list2[i]);
                if (sum < minSum) {
                    minSum = sum;
                    result.clear();
                    result.add(list2[i]);
    ...
    // LC1660
    Map<TreeNode52, TreeNode52> lc1660Parent;

    public TreeNode52 correctBinaryTree(TreeNode52 root) {
        lc1660Parent = new HashMap<>();
        Deque<TreeNode52> q = new LinkedList<>();
        q.offer(root);
        while (!q.isEmpty()) {
            TreeNode52 p = q.poll();
            if (lc1660Parent.get(p.right) != null) {
                TreeNode52 victim = p;
                TreeNode52 vp = lc1660Parent.get(victim);
    ...
    public int maxValue(TreeNode52 root, int k) {
        lcp34Limit = k;
        int[] result = lcp34Dfs(root);
        return Arrays.stream(result).max().getAsInt();
    }

    private int[] lcp34Dfs(TreeNode52 root) {
    ...
    public int[] findMode(TreeNode52 root) {
    ...
    private void lc501Dfs(TreeNode52 root) {
        if (root == null) return;
    ...
    // LCP10 Hard **
    public double minimalExecTime(TreeNode52 root) {
        double[] result = betterDfs(root);
        return result[1];
    }

    // 返回[任务总时间, 最短执行时间]
    private double[] betterDfs(TreeNode52 root) {
...
// https://leetcode-cn.com/problems/smallest-rotation-with-highest-score/solution/chai-fen-shu-zu-by-sssz-qdut/
    ...
    public boolean findTarget(TreeNode52 root, int k) {
        lc653Inorder(root);
    ...
    private void lc653Inorder(TreeNode52 root) {
        if (root == null) return;
    ...
    // JZOF 22
    public ListNode52 getKthFromEnd(ListNode52 head, int k) {
        ListNode52 fast = head, slow = head;
        for (int i = 0; i < k; i++) {
            fast = fast.next;
        }
        while (fast != null) {
...
class DisjointSetUnion52<T> {

    Map<T, T> father;
    Map<T, Integer> rank;

    public DisjointSetUnion52() {
        father = new HashMap<>();
        rank = new HashMap<>();
    }
...
class BIT52 {
    ...
    public BIT52(int len) {
        this.len = len;
        this.tree = new long[len + 1];
    }

    public BIT52(int[] arr) {
        this.len = arr.length;
        this.tree = new long[len + 1];
...
class RangeBit52 {
    ...
    public RangeBit52(int len) {
        diff = new BIT52(len);
        iDiff = new BIT52(len);
        this.len = len;
    }

    public RangeBit52(int[] arr) {
        this.len = arr.length;
        diff = new BIT52(len);
        iDiff = new BIT52(len);
