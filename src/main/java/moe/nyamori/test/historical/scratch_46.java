...
class scratch_46 {
    ...
    public boolean isRectangleCover(int[][] rectangles) {
        ...
        RangeBit46 bit = new RangeBit46(maxX - minX + 1);
    ...
    public int[] minInterval(int[][] intervals, int[] queries) {
        ...
        for (int i = 0; i < q.length; i++) {
            while (intvIdx < intervals.length && intervals[intvIdx][0] <= q[i][0]) {
                pq.offer(new int[]{intervals[intvIdx][0], intervals[intvIdx][1]});
                intvIdx++;
            }
            ...
            if (!pq.isEmpty()) {
                result[q[i][1]] = pq.peek()[1] - pq.peek()[0] + 1;
            } else {
                result[q[i][1]] = -1;
            }
        }
    ...
    public int maxAncestorDiff(TreeNode8 root) {
        lc1027Ts = new TreeMap<>();
        lc1027Helper(root);
        return lc1027Result;
    }

    private void lc1027Helper(TreeNode8 root) {
        if (root == null) {
            if (lc1027Ts.size() != 0) {
...
//        return step;
    ...
    // LC24
    public ListNode37 swapPairs(ListNode37 head) {
        ListNode37 dummy = new ListNode37(-1);
        dummy.next = head;
        ListNode37 cur = dummy;
        while (cur.next != null && cur.next.next != null) {
            ListNode37 close = cur.next;
            ListNode37 far = cur.next.next;

            ListNode37 farNext = far.next;
            far.next = close;
            close.next = farNext;
            cur.next = far;
    ...
    public TreeNode8 balanceBST(TreeNode8 root) {
        lc1382List = new ArrayList<>();
        inOrder(root);
        return buildBalanceBst(0, lc1382List.size() - 1);
    }

    private TreeNode8 buildBalanceBst(int start, int end) {
        if (start > end) return null;
        if (start == end) {
            return new TreeNode8(lc1382List.get(start));
        }

        int mid = (start + end) / 2;

        TreeNode8 root = new TreeNode8(lc1382List.get(mid));
    ...
    public int pathSumDfs(TreeNode8 root, int sum) {
    ...
    public int pathSumDfsHelper(TreeNode8 root, int sum) {
        if (root == null) return 0;
        int r = 0;
        if (root.val == sum) r = 1;
    ...
    public int pathSum(TreeNode8 root, int sum) {
        itv0412PrefixMap = new HashMap<>();
        itv0412PrefixMap.put(0, 1);
        return itv0412Helper(root, sum, 0);
    }

    private int itv0412Helper(TreeNode8 root, int target, int curSum) {
        if (root == null) return 0;
        int result = 0;
        curSum += root.val;
    ...
    public int[] findRedundantDirectedConnection(int[][] edges) {
        ...
        DisjointSetUnion46 dsu = new DisjointSetUnion46();
...
class TreeNode46 {
    ...
    TreeNode46(int val, TreeNode46 left, TreeNode46 right) {
        this.val = val;
        this.left = left;
        this.right = right;
    }
...
class ListNode46 {
    ...
    ListNode46(int val, ListNode46 next) {
        this.val = val;
        this.next = next;
    }
}
...
class Node46 {
    ...
    public Node46 topLeft;
    public Node46 topRight;
    public Node46 bottomLeft;
    public Node46 bottomRight;
    ...
    public Node46(boolean val, boolean isLeaf) {
        this.val = val;
        this.isLeaf = isLeaf;
        this.topLeft = null;
        this.topRight = null;
    ...
    public Node46(boolean val, boolean isLeaf, Node46 topLeft, Node46 topRight, Node46 bottomLeft, Node46 bottomRight) {
        this.val = val;
        this.isLeaf = isLeaf;
...
class Solution46 {
    public Node46 construct(int[][] grid) {
        return helper(grid, 0, grid.length - 1, 0, grid[0].length - 1);
    }

    private Node46 helper(int[][] grid, int up, int down, int left, int right) {
        Node46 n = new Node46();
...
class RangeBit46 { // 注意存入的是差分数组, 目标数组 a[i] 即对diff数组求前缀和
    long[] diff;
    long[] iDiff;
    int len;

    public RangeBit46(int len) {
        this.len = len;
        diff = new long[len + 1];
