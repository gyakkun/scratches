...
class scratch_47 {
    public static void main(String[] args) {
        ...
        rm.addRange(55, 62);
        rm.addRange(1, 29);
        rm.removeRange(18, 49);
        rm.queryRange(6, 98);
        rm.queryRange(59, 71);
        rm.removeRange(40, 45);
        rm.removeRange(4, 58);
        rm.removeRange(57, 69);
        rm.removeRange(20, 30);
        rm.removeRange(1, 40);
        rm.queryRange(73, 93);
        rm.removeRange(32, 93);
        rm.addRange(38, 100);
        rm.removeRange(50, 64);
        rm.addRange(26, 72);
        rm.queryRange(8, 74);
        rm.queryRange(15, 53);
        rm.addRange(44, 85);
        rm.addRange(10, 71);
        rm.queryRange(54, 70);
        rm.removeRange(10, 45);
        rm.queryRange(30, 66);
        rm.addRange(47, 98);
        rm.queryRange(1, 7);
        rm.removeRange(44, 78);
        rm.removeRange(31, 49);
        rm.addRange(62, 63);
        rm.addRange(49, 88);
        rm.removeRange(47, 72);
        rm.removeRange(8, 50);
        rm.removeRange(49, 79);
        rm.addRange(31, 47);
        rm.addRange(54, 87);
        rm.queryRange(77, 78);
        rm.queryRange(59, 100);
        rm.queryRange(8, 9);
        rm.queryRange(50, 51);
        rm.queryRange(67, 93);
        rm.removeRange(25, 86);
        rm.removeRange(8, 92);
        rm.queryRange(31, 87);
        rm.addRange(90, 95);
        rm.addRange(28, 56);
        ...
        rm.addRange(75, 81);
        rm.addRange(17, 63);
        rm.removeRange(78, 90);
        rm.addRange(9, 18);
        rm.queryRange(51, 74);
        rm.removeRange(20, 54);
        rm.addRange(35, 72);
        rm.queryRange(2, 29);
        rm.addRange(28, 41);
        rm.addRange(17, 95);
        ...
        rm.addRange(57, 96);
        rm.queryRange(51, 72);
        rm.removeRange(21, 67);
        rm.removeRange(40, 73);
        rm.removeRange(14, 26);
        rm.removeRange(71, 86);
        rm.queryRange(34, 41);
        ...
        rm.addRange(18, 93);
        rm.queryRange(13, 21);
        rm.removeRange(13, 46);
        rm.removeRange(56, 99);
        rm.queryRange(6, 93);
        rm.addRange(25, 36);
        rm.removeRange(27, 88);
    ...
    // LC143
    public void reorderList(ListNode47 head) {
        ListNode47 dummy = new ListNode47();
        dummy.next = head;
        ListNode47 fast = dummy.next;
        ListNode47 slow = dummy.next;
        while (fast.next != null && fast.next.next != null) {
            fast = fast.next.next;
            slow = slow.next;
        }
        boolean even = fast.next != null;
        ListNode47 middle = slow;
        ListNode47 secHalfDummyHead = new ListNode47();
        secHalfDummyHead.next = middle.next;
        middle.next = null;
        ListNode47 prev = null;
        ListNode47 cur = secHalfDummyHead.next;
        while (cur != null) {
            ListNode47 origNext = cur.next;
            cur.next = prev;
            prev = cur;
            cur = origNext;
        }
        ListNode47 secHalfRevDummyHead = new ListNode47();
        secHalfRevDummyHead.next = prev;

        ListNode47 firstPtr = dummy.next;
        ListNode47 secPtr = secHalfRevDummyHead.next;
        while (secPtr != null) {
            ListNode47 firstNext = firstPtr.next;
            ListNode47 secNext = secPtr.next;
    ...
    // LC86
    public ListNode47 partition(ListNode47 head, int x) {
        ListNode47 mainChainHead = new ListNode47();
        mainChainHead.next = head;
        ListNode47 subChainHead = new ListNode47();
        ListNode47 prev = mainChainHead;
        ListNode47 cur = mainChainHead.next;
        ListNode47 subChainPtr = subChainHead;
        while (cur != null) {
            if (cur.val >= x) {
                ListNode47 origNext = cur.next;
                cur.next = null;
    ...
    // LC1019
    public int[] nextLargerNodes(ListNode47 head) {
        ListNode47 dummyHead = new ListNode47();
        dummyHead.next = head;
        ListNode47 cur = dummyHead.next;
        Deque<ListNode47> stack = new LinkedList<>();
        int len = 0;
        while (cur != null) {
...
class CBTInserter47 {
    ...
    LinkedList<TreeNode8> lastButOneLayer;
    LinkedList<TreeNode8> lastLayer;
    TreeNode8 root;

    public CBTInserter47(TreeNode8 r) {
        LinkedList<TreeNode8> q = new LinkedList<>();
        q.offer(r);
        root = r;
        while (!q.isEmpty()) {
            int qSize = q.size();
            ...
            if (endFlag) {
                ...
                ListIterator<TreeNode8> it = lastButOneLayer.listIterator();
                while (it.hasNext()) {
                    TreeNode8 t = it.next();
    ...
    public int insert(int v) {
        TreeNode8 n = new TreeNode8(v);
        TreeNode8 p = lastButOneLayer.peek();
        lastLayer.offer(n);
        if (p.left == null) {
            p.left = n;
    ...
    public TreeNode8 get_root() {
        return root;
    }
}

// LC911 较慢
class TopVotedCandidate47 {
    TreeMap<Integer, Integer> timeCanMap;

    public TopVotedCandidate47(int[] persons, int[] times) {
...
class Codec47 {
    ...
    public String serialize(TreeNode8 root) {
        StringBuilder sb = new StringBuilder();
        preOrder(root, sb);
        sb.deleteCharAt(sb.length() - 1);
        return sb.toString();
    ...
    private void preOrder(TreeNode8 root, StringBuilder sb) {
        if (root == null) {
            sb.append("#,");
            return;
        }
    ...
    public TreeNode8 deserialize(String data) {
    ...
    private TreeNode8 preOrderDes(List<String> list) {
        if (list.get(0).equals("#")) {
            list.remove(0);
            return null;
        }
        TreeNode8 root = new TreeNode8(Integer.valueOf(list.get(0)));
        list.remove(0);
...
class ListNode47 {
    ...
    ListNode47(int val, ListNode47 next) {
        this.val = val;
        this.next = next;
    }
}
