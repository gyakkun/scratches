...
class scratch_60 {
    ...
    // LC156
    public TreeNode60 upsideDownBinaryTree(TreeNode60 root) {
        // 原来的左子节点变成新的根节点
        // 原来的根节点变成新的右子节点
        // 原来的右子节点变成新的左子节点
        // 题目保证如果存在右节点就存在左节点
        ...
        TreeNode60 origLeft = root.left, origRight = root.right;
        TreeNode60 newRoot = upsideDownBinaryTree(root.left);
    ...
    // LC1474
    public ListNode60 deleteNodes(ListNode60 head, int m, int n) {
        if (head == null) return null;
        ListNode60 dummy = new ListNode60(-1);
        dummy.next = head;
        ListNode60 cur = dummy;
        int idx = 0;
    ...
    // LC366
    public List<List<Integer>> findLeaves(TreeNode60 root) {
    ...
    private void lc366Helper(TreeNode60 node, List<Integer> tmp) {
        if (node == null) return;
        if (node.left != null) {
    ...
    // LC1171
    ListNode60 victim = null;

    public ListNode60 removeZeroSumSublists(ListNode60 head) {
        victim = head;
        while (handle(victim)) {
            ;
        }
    ...
    private boolean handle(ListNode60 head) {
        ListNode60 dummy = new ListNode60(0);
        Map<Integer, ListNode60> prefixSumNodeMap = new HashMap<>();
        dummy.next = head;
        ListNode60 it = dummy;
        int sum = 0;
        while (it != null) {
            ...
            if (prefixSumNodeMap.containsKey(sum)) {
                ListNode60 prev = prefixSumNodeMap.get(sum);
                prev.next = it.next;
                victim = dummy.next;
                return true;
            }
    ...
    public int findTilt(TreeNode60 root) {
        lc563Helper(root);
        return lc563Result;
    }

    public int lc563Helper(TreeNode60 root) {
    ...
    public int pathSumIii(TreeNode60 root, int targetSum) {
        lc437Prefix = new HashMap<>();
        lc437Prefix.put(0, 1); // root, sum=0
        return lc437Helper(root, 0, targetSum);
    }

    private int lc437Helper(TreeNode60 root, int cur, int target) {
        if (root == null) return 0;
        cur += root.val;
    ...
    public int maxPathSum(TreeNode60 root) {
        lc124Helper(root);
        return lc124Result;
    }

    private int lc124Helper(TreeNode60 root) {
        if (root == null) return 0;
    ...
    public List<List<Integer>> pathSum(TreeNode60 root, int targetSum) {
        lc113Result = new ArrayList<>();
        lc113Helper(root, targetSum, new ArrayList<>());
        return lc113Result;
    }

    private void lc113Helper(TreeNode60 root, int target, List<Integer> path) {
        if (root == null) return;
        path.add(root.val);
    ...
    // LC508
    public int[] findFrequentTreeSum(TreeNode60 root) {
    ...
    private int lc508Helper(TreeNode60 root, Map<Integer, Integer> freq) {
        if (root == null) return 0;
...
class ListNode60 {
    ...
    ListNode60(int val, ListNode60 next) {
        this.val = val;
        this.next = next;
    }
}
