...
class scratch_56 {
    ...
    // LC272
    public List<Integer> closestKValues(TreeNode56 root, double target, int k) {
        PriorityQueue<Integer> pq = new PriorityQueue<>(Comparator.comparingDouble(o -> -Math.abs(target - (double) o)));
        Deque<TreeNode56> q = new LinkedList<>();
        q.offer(root);
        while (!q.isEmpty()) {
            TreeNode56 p = q.poll();
    ...
    public int pseudoPalindromicPaths(TreeNode56 root) {
        lc1457Helper(root);
        return lc1457Result;
    }

    private void lc1457Helper(TreeNode56 root) {
        if (root == null) return;
    ...
    public boolean[] distanceLimitedPathsExist(int n, int[][] edgeList, int[][] queries) {
        ...
        DSUArray34 dsu = new DSUArray34(n);
    ...
    // LC814
    public TreeNode56 pruneTree(TreeNode56 root) {
        if (!subtreeHasOne(root)) return null;
        lc814Helper(root);
        return root;
    }

    private void lc814Helper(TreeNode56 root) {
        if (root == null) return;
    ...
    private boolean subtreeHasOne(TreeNode56 root) {
...
class Trie56 {
    ...
    public boolean addWord(String word) {
        ...
        for (char c : word.toCharArray()) {
            if (!cur.children.containsKey(c)) {
                cur.children.put(c, new TrieNode56());
            }
    ...
    public boolean search(String word) {
        TrieNode56 target = getNode(word);
    ...
    private TrieNode56 getNode(String prefix) {
        TrieNode56 cur = root;
...
class TrieNode56 {
    Map<Character, TrieNode56> children = new HashMap<>();
    int end = 0;
    int path = 0;
}
...
class DisjointSetUnion56<T> {

    Map<T, T> father;
    Map<T, Integer> rank;

    public DisjointSetUnion56() {
        father = new HashMap<>();
        rank = new HashMap<>();
    }
...
class CodecLc431 {
    ...
    public TreeNode56 encode(Node root) {
        if (root == null) return null;
        TreeNode56 tn = new TreeNode56(root.val);
        if (root.children.size() == 0) return tn;
        TreeNode56 rightForChildren = tn;
        TreeNode56 siblings = null, firstSiblings = null;
        for (Node c : root.children) {
            TreeNode56 cn = encode(c);
            if (siblings == null) {
                siblings = cn;
                firstSiblings = cn;
                continue;
    ...
    public Node decode(TreeNode56 root) {
        if (root == null) return null;
        ...
        TreeNode56 siblings = root.right;
        while (siblings != null) {
