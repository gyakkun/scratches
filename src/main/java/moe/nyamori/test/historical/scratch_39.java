...
//        }
    ...
    // LC328
    public ListNode37 oddEvenList(ListNode37 head) {
        ListNode37 first = head;
        if (first == null || first.next == null) return head;
        ListNode37 second = head.next;
        ListNode37 secondDummy = new ListNode37(-1);
    ...
    public TreeNode8 lowestCommonAncestor(TreeNode8 root, TreeNode8 p, TreeNode8 q) {
        Map<TreeNode8, TreeNode8> parent = new HashMap<>();
        Set<TreeNode8> visited = new HashSet<>();
        Deque<TreeNode8> queue = new LinkedList<>();
        queue.offer(root);
        parent.put(root, null);
        // 层次遍历 取得所有父子关系
        while (!queue.isEmpty()) {
            TreeNode8 tmp = queue.poll();
            if (tmp.left != null) {
...
//        dp[0][0] = true;
    ...
    public boolean isPalindrome(ListNode37 head) {
        // get mid
        ListNode37 dummy = new ListNode37();
        dummy.next = head;
        ListNode37 middleDummy = new ListNode37();
    ...
    public int kthSmallest(TreeNode8 root, int k) {
        inorder(root, k);
        return lc230Result;
    }

    private void inorder(TreeNode8 root, int k) {
...
class TreeNode39 {
    ...
    TreeNode39(int val, TreeNode8 left, TreeNode8 right) {
        this.val = val;
        this.left = left;
        this.right = right;
    }
...
class ListNode39 {
    ...
    ListNode39(int val, ListNode37 next) {
        this.val = val;
        this.next = next;
    }
}
