...
class scratch_36 {
    ...
    // LC897
    public TreeNode8 increasingBST(TreeNode8 root) {
        Deque<TreeNode8> stack = new LinkedList<>();
        List<TreeNode8> q = new LinkedList<>();
        stack.push(root);
        while (!stack.isEmpty()) {
...
//        }
    ...
    public Node36 connectSolution(Node36 root) {
        if (root == null) {
            return root;
        }
        // 从根节点开始
        Node36 leftmost = root;
        while (leftmost.left != null) {
            // 遍历这一层节点组织成的链表，为下一层的节点更新 next 指针
            Node36 head = leftmost;
            while (head != null) {
                // CONNECTION 1
    ...
    // LC116, O(n) space
    public Node36 connect(Node36 root) {
        if (root == null) {
            return null;
        }
        Deque<Node36> q = new LinkedList<>();
        q.offer(root);
        while (!q.isEmpty()) {
            int len = q.size();
            for (int i = 0; i < len; i++) {
    ...
    private TreeNode8 sortedArrayToBSTHelper(int[] nums, int left, int right) {
        if (left == right) return new TreeNode8(nums[left]);
    ...
    public TreeNode8 buildTreeHelper(int[] preorder, int[] inorder, int preorderLeft, int preorderRight, int inorderLeft, int inorderRight) {
        ...
        // 先把根节点建立出来
        TreeNode8 root = new TreeNode8(preorder[preorderRoot]);
        // 得到左子树中的节点数目
    ...
    public TreeNode8 buildTree(int[] preorder, int[] inorder) {
    ...
    // LC101
    public boolean isSymmetric(TreeNode8 root) {
        return isSymmerticHelper(root, root);
    }

    private boolean isSymmerticHelper(TreeNode8 a, TreeNode8 b) {
    ...
    // LC101 solution
    public boolean check(TreeNode8 u, TreeNode8 v) {
        Queue<TreeNode8> q = new LinkedList<TreeNode8>();
        q.offer(u);
        q.offer(v);
        while (!q.isEmpty()) {
            u = q.poll();
...
class TreeNode36 {
    ...
    TreeNode36(int val, TreeNode8 left, TreeNode8 right) {
        this.val = val;
        this.left = left;
        this.right = right;
    }
...
class Node36 {
    ...
    public Node36(int _val, Node36 _left, Node36 _right, Node36 _next) {
        val = _val;
        left = _left;
        right = _right;
        next = _next;
...
class DisjointSetUnion36 {

    Map<Integer, Integer> father;
    Map<Integer, Integer> rank;

    public DisjointSetUnion36() {
        father = new HashMap<>();
        rank = new HashMap<>();
    }
