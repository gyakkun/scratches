...
class Solution {
    ...
    private static ListNode37 prepareNode(int[] arr) {
        ListNode37 root = new ListNode37(arr[0]);
        ListNode37 pivot = root;
        for (int i = 1; i < arr.length; i++) {
            pivot.next = new ListNode37(arr[i]);
            pivot = pivot.next;
        }
        return root;
    }
    ...
    public int[][] spiralMatrix(int m, int n, ListNode37 head) {
        int[][] res = new int[m][n];
        for (int i = 0; i < m; i++) for (int j = 0; j < n; j++) res[i][j] = -1;
        BitSet bs = new BitSet(m * n);
        int dir = 0;
        ListNode37 node = head;
        int curX = 0, curY = 0;
        while (node != null) {
...
class ListNode {
    ...
    ListNode(int val, ListNode37 next) {
        this.val = val;
        this.next = next;
    }
}
