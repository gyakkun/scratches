...
class scratch_44 {
    ...
    // LC145 后序遍历 迭代
    public List<Integer> postorderTraversal(TreeNode8 root) {
        List<Integer> result = new LinkedList<>();
        Deque<TreeNode8> stack = new LinkedList<>();
        TreeNode8 lastVisit = root;
    ...
    // LC94 中序遍历 迭代
    public List<Integer> inorderTraversal(TreeNode8 root) {
        List<Integer> result = new LinkedList<>();
        Deque<TreeNode8> stack = new LinkedList<>();
        while (root != null || !stack.isEmpty()) {
            while (root != null) {
    ...
    // LC817
    public int numComponents(ListNode44 head, int[] nums) {
        Set<Integer> g = new HashSet<>();
        for (int i : nums) {
            g.add(i);
        }
        ListNode44 dummy = new ListNode44(-1);
        dummy.next = head;
        boolean inGear = false;
        ListNode44 ptr = dummy;
        int result = 0;
        while (ptr != null) {
    ...
    // LC1367
    // Map<Pair<ListNode44, TreeNode>, Boolean> lc1367Memo;

    public boolean isSubPath(ListNode44 head, TreeNode8 root) {
    ...
    private boolean lc1367Helper(ListNode44 cur, TreeNode8 root, ListNode44 head) {
        if (cur == null) return true;
        if (root == null) return false;
        //  Pair<ListNode44, TreeNode> status = new Pair<>(cur, root);
    ...
    // LC669
    public TreeNode8 trimBST(TreeNode8 root, int low, int high) {
        if (root == null) return null;
        TreeNode8 left = root.left, right = root.right; // 注意提前存下引用
...
class ListNode44 {
    ...
    ListNode44(int val, ListNode44 next) {
        this.val = val;
        this.next = next;
    }
}