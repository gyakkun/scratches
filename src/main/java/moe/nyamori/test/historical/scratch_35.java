...
class scratch_35 {
    public static void main(String[] args) {
        scratch_35 s = new scratch_35();
        System.err.println(s.maxCoins(new int[]{3, 1, 5, 8}));
    }

    // LC337 树形DP
    public int rob(TreeNode8 root) {
        // 构造两个map<node,int>, 分别存储rob该节点和不rob该节点能取到的最大值
        Map<TreeNode8, Integer> choose = new HashMap<>();
        Map<TreeNode8, Integer> notChoose = new HashMap<>();
        choose.put(null, 0);
    ...
    private void robLc337Helper(TreeNode8 root, Map<TreeNode8, Integer> choose, Map<TreeNode8, Integer> notChoose) {
...
class TrieNode35 {
    ...
    public TrieNode35(Character c) {
        this.val = c;
        this.isEnd = false;
    }

    public TrieNode35 searchSibling(Character c) {
        TrieNode35 former = this;
    ...
    public TrieNode35 searchChildren(Character c) {
        TrieNode35 former = this;
...
class TreeNode35 {
    ...
    TreeNode35(int val, TreeNode8 left, TreeNode8 right) {
        this.val = val;
        this.left = left;
        this.right = right;
    }
